							...
							>
								{!isMe && (
									<CPAvatar
										src={
											m.avatar ||
											`https://ui-avatars.com/api/?name=${encodeURIComponent(m.authorName)}`
										}
										alt={m.authorName}
									/>
								)}
									...
									<div className={`cp-bubble ${isMe ? "me" : "them"}`}>
										...
										{isHovered && (
											<CPHoverActions>
												...
												<CPRepBtn onClick={() => toggleThread(m.id)}>
													Reply
												</CPRepBtn>
											</CPHoverActions>
										)}
									...
									<CPThreadPreviewRow>
										{m.replyCount ? (
											<CPThreadPreview onClick={() => toggleThread(m.id)}>
												...
												<CPThreadText>
													{m.replyCount} replies — open thread
												</CPThreadText>
											</CPThreadPreview>
										) : null}
									</CPThreadPreviewRow>
											...
											<CPThreadList>
												{(m.replies || []).map((r) => (
													...
													>
														...
														<CPReplyContent style={{ whiteSpace: "pre-line" }}>
															{r.content}
														</CPReplyContent>
													</div>
												))}
