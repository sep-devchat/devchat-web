...
export const MessageStyleOption = styled.div<{ $selected: boolean }>`
  flex: 1;
  padding: 12px;
  border: ${({ $selected }) => $selected ? '2px solid #133E87' : '1px solid #d1d5db'};
  border-radius: 8px;
  cursor: pointer;
  transition: all 0.2s ease;
  position: relative;
...
export const MessageDisplayRadio = styled.div<{ $selected: boolean }>`
  ...
  flex-shrink: 0;
  margin-top: 2px;
  background-color: ${({ $selected }) => $selected ? '#133E87' : 'transparent'};
...
export const ClickableChatPreview = styled.div<{ $selected: boolean }>`
  ...
  border-radius: 8px;
  border: ${({ $selected }) => $selected ? '2px solid #608BC1' : '1px solid #e5e7eb'};
  cursor: pointer;
  transition: all 0.2s ease;
