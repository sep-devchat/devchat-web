...
const AddFriend: React.FC<Props> = ({
	...
	return (
		<>
			...
			<Subtitle>You can find and add friends with their email/username</Subtitle>
			...
			{searchResults.length > 0 && !isUserSelectedFromList && (
				<ResultsList>
					{searchResults.map((user) => (
						<ResultItem key={user.id} onClick={() => onSelectUser(user)} style={{ position: 'relative' }}>
							...
							<button
								...
								onMouseOver={(e) => !loadingProfile && (e.currentTarget.style.background = '#DBEAFE')}
								onMouseOut={(e) => !loadingProfile && (e.currentTarget.style.background = '#EFF6FF')}
							>
								<Eye size={18} />
							</button>
						</ResultItem>
